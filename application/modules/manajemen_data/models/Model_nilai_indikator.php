...
class Model_nilai_indikator extends CI_Model
{
    ...
    public function insertDataIndikatorLink()
    {
        ...
        $nama_thumbnail      = escape($this->input->post('nama_thumbnail', TRUE));

        //cek nama tematik duplicate
        $this->db->where('link_youtube', $link_youtube);
        $this->db->where('token', $tokenInovasi);
        $checkData = $this->db->count_all_results('ms_indikator_link');
        if ($checkData > 0) {
            return array('response' => 'ERROR', 'name' =>  '');
        } else {
            $year    = date('Y');
            $month   = date('m');
            $dirname = 'dokumen/thumbnails/' . $year . '/' . $month . '/';
            if (!is_dir($dirname)) {
                mkdir('./' . $dirname, 0777, TRUE);
            }

            $_FILES['file']['name']     = $_FILES['nama_thumbnail']['name'];
            $_FILES['file']['type']     = $_FILES['nama_thumbnail']['type'];
            $_FILES['file']['tmp_name'] = $_FILES['nama_thumbnail']['tmp_name'];
            $_FILES['file']['error']    = $_FILES['nama_thumbnail']['error'];
            $_FILES['file']['size']     = $_FILES['nama_thumbnail']['size'];
            $config = array(
                'upload_path'      => './' . $dirname . '/',
                'allowed_types'    => 'png|jpg|jpeg|pdf',
                'file_name'        => 'file_' . $tokenInovasi . '_' . date('YmdHis', strtotime($create_date)),
                'file_ext_tolower' => TRUE,
                'max_size'         => 5024,
                'max_filename'     => 0,
                'remove_spaces'    => TRUE,
            ...
            if (!$this->upload->do_upload('file')) {
                ...
                $upload_data = $this->upload->data();
                $nama_thumbnail   = $upload_data['file_name'];
            }
            $data = array(
            ...
            return array('response' => 'SUCCESS', 'name' =>  '');
        }
    }
