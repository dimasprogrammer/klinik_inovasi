...
class Model_inovasi extends CI_Model
{
    ...
    var $search = array('a.nama_inovasi');
    ...
    private function _get_datatables_query($id_tahapan_inovasi, $param)
    {
        ...
        $this->db->order_by('a.id_inovasi DESC');
    }
    ...
    public function getDataDetailPermohonanInformasi($token)
    {
    ...
    public function updateDataInovasi()
    {
        ...
        foreach ($upload as $key => $i) {
            ...
            if (!$this->upload->do_upload('file')) {
                ...
                $this->unlink_data('dokumen/inovasi/', $year, $month, $nameFile);
                $upload_data = $this->upload->data();
                $nama_file   = $upload_data['file_name'];
            }
    ...
    /* tematik untuk insert data */
    public function updateDataPermohonan()
    {
        //get data
        $iduser            = $this->app_loader->current_userid();
        $create_date       = gmdate('Y-m-d H:i:s', time() + 60 * 60 * 7);
        $create_ip         = $this->input->ip_address();
        $token             = escape($this->input->post('tokenId', TRUE));
        $nama_lengkap_update      = escape($this->input->post('nama_lengkap_update', TRUE));
        $jabatan_update           = escape($this->input->post('jabatan_update', TRUE));
        $instansi_update          = escape($this->input->post('instansi_update', TRUE));
        $no_hp_update             = escape($this->input->post('no_hp_update', TRUE));
        $email_update             = escape($this->input->post('email_update', TRUE));
        $status_permohonan_update = escape($this->input->post('status_permohonan_update', TRUE));

        //cek nama tematik duplicate
        $data = array(
            'token'             => $token,
            'nama_lengkap'      => $nama_lengkap_update,
            'jabatan'           => $jabatan_update,
            'instansi'          => $instansi_update,
            'no_hp'             => $no_hp_update,
            'email'             => $email_update,
            'status_permohonan' => $status_permohonan_update,
            'create_by'         => $iduser,
            'create_date'       => $create_date,
            'create_ip'         => $create_ip,
            'mod_by'            => $iduser,
        ...
        /*query insert*/
        $this->db->where('token', $token);
        $this->db->update('ms_permohonan', $data);
    ...
    /*Fungsi get data edit by id*/
    public function getDataInovasiCetakPDF($token)
    {
        $this->db->select('a.id_inovasi,
                           a.token,
                           a.nama_inovasi,
        ...
        $this->db->join('cx_urusan_utama b', 'a.id_urusan_utama = b.id_urusan_utama', 'inner');
        $this->db->join('xi_sa_users c', 'a.create_by = c.token', 'inner');
        $this->db->join('cx_bentuk_inovasi d', 'a.id_bentuk_inovasi = d.id_bentuk_inovasi', 'inner');
    ...
    public function getTotalBobotByIdInovasi($token)
    {
        ...
        // return $total_bobot;
        if ($this->getFileCheck($token) == 20)
            return $total_bobot;
        else return $total_bobot;
    }
    ...
    public function cekDokumenLink($tokenInovasi)
    {
        $this->db->select('a.id_indikator_link, a.create_date, a.thumbnail, a.link_youtube');
