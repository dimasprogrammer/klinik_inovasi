...
<script type="text/javascript">
    ...
    $(document).on('click', '.btnPermohonanUpdate', function(e) {
        formReset();
        $('#formEntryPermohonanUpdate').attr('action', site + '/updatePermohonan');
        var token = $(this).data('id');
        // alert(token);
        $('#modalEntryFormPermohonanUpdate').modal({
            backdrop: 'static'
        });
        getDataIndikatorPermohonanUpdate(token);
    });

    function getDataIndikatorPermohonanUpdate(token) {
        run_waitMe($('#frmEntryPermohonanUpdate'));
        $.ajax({
            type: 'POST',
            url: site + '/detailPermohonanUpdate',
            data: {
                'token': token,
                '<?php echo $this->security->get_csrf_token_name(); ?>': $('input[name="' + csrfName + '"]').val()
            },
            ...
            success: function(data) {
                ...
                if (data.status == 'RC200') {
                    $('input[name="tokenId"]').val(token);
                    $('#nama_lengkap_update').val(data.message.nama_lengkap);
                    $('#jabatan_update').val(data.message.jabatan);
                    $('#instansi_update').val(data.message.instansi);
                    $('#no_hp_update').val(data.message.no_hp);
                    $('#email_update').val(data.message.email);
                }
                $('#frmEntryPermohonanUpdate').waitMe('hide');
            }
        });
    }

    $(document).on('submit', '#formEntryPermohonanUpdate', function(e) {
        e.preventDefault();
        // let postData = $(this).serialize();
        // get form action url
        var form = $('#formEntryPermohonanUpdate')[0];
        let formActionURL = $(this).attr("action");
        $("#savePermohonanUpdate").html('<i class="spinner-grow spinner-grow-sm mr-2" role="status" aria-hidden="true"></i> DIPROSES...');
        $("#savePermohonanUpdate").addClass('disabled');
        // alert(formActionURL);
        run_waitMe($('#frmEntryPermohonanUpdate'));
        swalAlert.fire({
            title: 'Konfirmasi',
            ...
            if (result.value) {
                $.ajax({
                    ...
                    $('input[name="' + csrfName + '"]').val(data.csrfHash);
                    if (data.status == 'RC404') {
                        $('#formEntryPermohonanUpdate').addClass('was-validated');
                        $('.invalid-feedback').removeClass('valid-feedback').text('');
                        swalAlert.fire({
                            title: 'Gagal Simpan',
                            text: 'Proses simpan data gagal, silahkan diperiksa kembali',
                            ...
                            if (result.value) {
                                $('#errEntryPermohonanUpdate').html(msg.error('Silahkan dilengkapi data pada form inputan dibawah'));
                                $.each(data.message, function(key, value) {
                                    if (key != 'isi')
                                        $('input[name="' + key + '"], textarea[name="' + key + '"], select[name="' + key + '"]').closest('div.required').find('div.invalid-feedback').addClass('valid-feedback').text(value);
                                    else {
                    ...
                    $('#errEntryPermohonanUpdate').html(msg.error('Harap periksa kembali data yang diinputkan'));
                    $('#frmEntryPermohonanUpdate').waitMe('hide');
                }).always(function() {
                    $("#savePermohonanUpdate").html('<i class="fas fa-check"></i> SUBMIT');
                    $("#savePermohonanUpdate").removeClass('disabled');
                });
            } else if (result.dismiss === Swal.DismissReason.cancel) {
                swalAlert.fire({
                    title: 'Batal Simpan',
    ...
    //     url = site + '/export_to_excel?token=' + token;
